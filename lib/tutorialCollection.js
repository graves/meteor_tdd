...
Tutorial.prototype = {
  ...
  registerStudent: function(studentId) {
    ...
    if (this.currentCapacity >= this.capacity) {
      throw "Capacity of tutorial has been reached!";
    }
    // check for existing registrations
    if (Tutorials.findOne({studentId: studentId}) !== null) {
  ...
  removeRegistration: function(studentId) {
    var tutorialReg = TutorialRegistrations.findOne({tutorialId: this.id, studentId: studentId});
    if (tutorialReg == null) {
      throw "Student not registered!";
    }

    TutorialRegistrations.remove({tutorialId: this.id, studentId: studentId});
